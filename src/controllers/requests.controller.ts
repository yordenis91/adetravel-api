...
export async function getRequest(req: Request, res: Response): Promise<void> {
  const item = await prisma.request.findUnique({ where: { id: String(req.params.id) }, include: { client: true } });
...
export async function getRequestQuotations(req: Request, res: Response): Promise<void> {
  const data = await prisma.quotation.findMany({ where: { requestId: String(req.params.id) }, orderBy: { createdAt: "desc" } });
...
export async function getRequestPayments(req: Request, res: Response): Promise<void> {
  const data = await prisma.payment.findMany({ where: { requestId: String(req.params.id) }, orderBy: { createdAt: "desc" } });
  sendList(res, data, data.length, 1, data.length || 1);
}

export async function getRequestVouchers(req: Request, res: Response): Promise<void> {
  const data = await prisma.voucher.findMany({ where: { requestId: String(req.params.id) }, orderBy: { createdAt: "desc" } });
  sendList(res, data, data.length, 1, data.length || 1);
}
...
export async function deleteRequest(req: Request, res: Response): Promise<void> {
  const item = await prisma.request.delete({ where: { id: String(req.params.id) } });
